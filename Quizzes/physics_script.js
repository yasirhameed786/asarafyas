...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Fetch Physics questions from API
    async function fetchPhysicsQuestions() {
        try {
            const response = await fetch('https://opentdb.com/api.php?amount=10&category=19&type=multiple'); // Category 19 represents Physics questions
    ...
    async function initializeQuiz() {
        try {
            questions = await fetchPhysicsQuestions();
            displayQuestions();
        } catch (error) {
