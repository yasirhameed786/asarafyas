...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Fetch General Knowledge questions from API
    async function fetchGeneralKnowledgeQuestions() {
        try {
            const response = await fetch('https://opentdb.com/api.php?amount=10&category=9&type=multiple'); // Category 9 represents General Knowledge questions
    ...
    async function initializeQuiz() {
        try {
            questions = await fetchGeneralKnowledgeQuestions();
            displayQuestions();
        } catch (error) {
